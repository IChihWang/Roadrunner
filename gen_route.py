...
def generate_routefile(arrival_rate):
    ...
    #dir_prob = [0.1, 0.1, 0.1, 0.1]
    #dir_prob = [0.2, 0.2, 0.2, 0.2]
    #dir_prob = [0.3, 0.3, 0.3, 0.3]
    #dir_prob = [0.4, 0.4, 0.4, 0.4]
    #dir_prob = [0.5, 0.5, 0.5, 0.5]
    #dir_prob = [0.6, 0.6, 0.6, 0.6]
    #dir_prob = [0.7, 0.7, 0.7, 0.7]
    #dir_prob = [0.8, 0.8, 0.8, 0.8]

    #dir_prob = [0.9, 0.9, 0.9, 0.9]
    #dir_prob = [1.0, 1.0, 1.0, 1.0]
    print(dir_prob)
    ...
    with open("data/icacc+.rou.xml", "w") as routes:
        ...
        for i in range(5,10):
            vType_str = '\t<vType id="car%i" accel="100.0" decel="100.0" sigma="0.0" length="%i" minGap="1.0" maxSpeed="%f" carFollowModel="CACC" color="255,255,255"/>' % (i, i, cfg.MAX_SPEED);
        ...
        for i in range(cfg.N_TIME_STEP):
            for idx in range(4):
                if random.uniform(0, 1) < float(dir_prob[idx]):
